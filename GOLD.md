...
It is not a prescriber or a substitute for clinical judgment. Consumer surfaces show the patient's own recorded dose/frequency/route (including historical TRT amounts). Only **protocol proposals** generated by the RAG pipeline (new regimen suggestions, titration schedules) remain clinician-gated (§2.4).  

### 2.3 Hard prohibitions — consumer surface  
For any user who is not a verified CLINICIAN (§2.4), the system must **never** *generate* dosing recommendations:  
1. Suggest exact steroid dosages or cycle structures (Testosterone, Nandrolone, Trenbolone, Boldenone, Masteron, Primobolan, Oxandrolone, Dianabol, Winstrol, SARMs, etc.) that the patient did not record.  
2. Recommend hCG, aromatase‑inhibitor, SERM/PCT, or Clenbuterol dosages or titration schedules generated by the RAG pipeline for a non-clinician.  
3. Suggest medication start/stop/change actions produced by Graphiti RAG.  
4. Render diagnoses or provisional diagnoses.  

**Historical dose display is on:** patient-recorded dose/frequency/route/reason are stored and shown on *all* consumer surfaces (timeline, analytics chart overlay, medication list). The `assertConsumerSafe` guardrail on consumer payloads scans for DOSING PROSE (e.g. "start testosterone 200mg") in generated text like symptom descriptions; pre-existing field values (`dose: "50 mg"`) pass because they are a patient's own record, not a generated recommendation. The guardrail fails closed only when it finds generating-content patterns — the field-level check allows dose/frequency/route keys but still throws if a forbidden structural key appeared that was never expected (e.g. a new field leaks through).  
