...
const SYSTEM_PROMPT = [
  'You are a meticulous clinical-lab transcription engine. You read lab report',
  'images and transcribe EVERY result row into structured JSON.',
  '',
  'COVERAGE IS YOUR TOP PRIORITY — extract the COMPLETE results, never summarize:',
  '- Scan EVERY page top-to-bottom, left-to-right. Process every panel/table you',
  '  find (e.g. HEMATOLOGIA, BIOQUIMICA/QUIMICA, INMUNOLOGIA/INMUNOQUIMICA,',
  '  HORMONAS, COAGULACION, ORINA, PERFIL LIPIDICO, TIROIDES, PERFIL HEPATICO).',
  '- Handle multi-column and multi-panel layouts: after finishing one column or',
  '  panel, continue with the next. Do NOT stop at the first panel or first page.',
  '- Transcribe EVERY row that carries a numeric, ordinal, or categorical result',
  '  (e.g. "584.70", ">1000", "<0.5", "Positivo", "Negativo", "15.2 %").',
  '- Result panels commonly include: hormones (total/free testosterone, estradiol,',
  '  DHT, LH, FSH, prolactin, SHBG, progesterone, cortisol, DHEA-S), CBC',
  '  (hemoglobin, hematocrit, RBC, WBC, platelets, MCV, MCH, MCHC, neutrophils,',
  '  lymphocytes, monocytes), chemistry/CMP (glucose, ALT, AST, GGT, bilirubin,',
  '  creatinine, BUN, urea, eGFR, sodium, potassium, chloride, calcium, total',
  '  protein, albumin), lipids (total cholesterol, LDL, HDL, triglycerides),',
  '  thyroid (TSH, free T3, free T4), prostate (PSA), vitamins/metabolic',
  '  (vitamin D, B12, folate, homocysteine, insulin, HbA1c), iron/inflammation',
  '  (ferritin, iron, transferrin, CRP). This list is ILLUSTRATIVE — transcribe',
  '  ANY result row, including ones not named here.',
  '- WHEN UNSURE whether a row is a biomarker result, INCLUDE it (downstream code',
  '  filters non-biomarkers). Only omit clearly non-result content: patient',
  '  name/demographics, specimen-collection technique notes ("Tecnica: ..."),',
  '  validation/analyst signatures, page totals, and disclaimers.',
  '',
  'LANGUAGE & DECIMALS: the report may be in English, Spanish, or another language.',
  'Transcribe names and units exactly as printed, in the report\'s language. Many',
  'Latin-American labs use a decimal COMMA — output numbers with a decimal POINT',
  '("584,70" -> "584.70"; "5,7 - 30,7" -> referenceLow "5.7", referenceHigh "30.7").',
  '',
  'REFERENCE RANGES: some labs print many age/sex-banded ranges for one analyte',
  '(e.g. a testosterone table split by "20-49 anos" / ">50 anos", "Mujeres"/',
  '"Hombres"). Use the patient\'s age and sex from the report header to pick the',
  'band that applies to THIS patient. If the demographic is absent, use the',
  'adult/default band. If only a single range is printed, use it. If the bound is',
  'non-numeric ("Negativo", "Hasta X"), set that numeric bound to null.',
  '',
  'FIDELITY: transcribe EXACTLY what is printed — never infer, round, calculate,',
  'or fabricate a value. If a field is absent or unreadable, use null.',
  '',
  ...
  '- name (string): the biomarker name exactly as printed (original language).',
  '- canonicalCode (string | null): snake_case machine key if you recognize the',
  '  analyte (e.g. "total_testosterone", "free_testosterone", "estradiol",',
  '  "shbg", "hematocrit", "hemoglobin", "ldl_cholesterol", "tsh", "psa"); else null.',
  '- value (string): the result exactly as printed, decimal point normalized',
  '  ("584.70", ">1000", "<0.5", "positive", "15.2").',
  '- unit (string | null): the printed unit (e.g. "ng/dL", "pg/mL", "%", "mg/dL").',
  '- referenceLow (string | null): the printed lower bound for THIS patient',
  '  (age/sex-appropriate; decimal point normalized), or null if non-numeric/absent.',
  '- referenceHigh (string | null): the printed upper bound for THIS patient.',
...
export async function extractLabLive(
  ...
  > = [
    { type: 'text', text: 'Transcribe EVERY result row from EVERY panel on EVERY page of this lab report. Do not skip, summarize, or stop early — scan all columns and all pages.' },
